...
struct
    ...
    fun exp1 (InFrame(i)) = exp (InFrame(i)) (Tree.TEMP(FP))
      | exp1 (InReg(reg)) = exp (InReg(reg)) (Tree.TEMP(FP))

    fun externalCall(s, args) = Tree.CALL(Tree.NAME(Temp.namedlabel s), args)

    fun intify(InFrame(x)) = x
      | intify(InReg(x)) = 0

    fun procEntryExit1(frame,body) = let
            val s0a = allocLocal(frame)(true)
            val s1a = allocLocal(frame)(true)
            val s2a = allocLocal(frame)(true)
            val s3a = allocLocal(frame)(true)
            val s4a = allocLocal(frame)(true)
            val s5a = allocLocal(frame)(true)
            val s6a = allocLocal(frame)(true)
            val s7a = allocLocal(frame)(true)
        ...
        in
            Tree.SEQ[
                ...
                Tree.MOVE(exp1(s3a), Tree.TEMP(S3)),
                Tree.MOVE(exp1(s4a), Tree.TEMP(S4)),
                Tree.MOVE(exp1(s5a), Tree.TEMP(S5)),
                Tree.MOVE(exp1(s6a), Tree.TEMP(S6)),
                Tree.MOVE(exp1(s7a), Tree.TEMP(S7)),
                Tree.MOVE(exp1(raa), Tree.TEMP(RA)),
                body,
                Tree.MOVE(Tree.TEMP(S0), exp1(s0a)),
                Tree.MOVE(Tree.TEMP(S1), exp1(s1a)),
                Tree.MOVE(Tree.TEMP(S2), exp1(s2a)),
                Tree.MOVE(Tree.TEMP(S3), exp1(s3a)),
                Tree.MOVE(Tree.TEMP(S4), exp1(s4a)),
