...
struct
   ...
   fun funcBody (name, params: A.field list, result, pos, level) =
	    ...
	    val names' = map #name params
            ...
            fun enterparam ({name,escape,ty},v) =
                S.enter(v,name,Env.VarEntry{access=TR.allocLocal(level)(!escape), ty=ty})
   ...
   and rightrec(x, T.NAME(sym, ty)) = if T.NAME(sym, ty) = x then true else (case !ty of
    ...
    and transDec(venv, tenv, dec, break, level) = 
    	let
         ...
         val trbody = transExp(venv, tenv, body, break, nextLevel) in (if checkTypeEqual(rt, #ty(trbody)) then () else E.error pos "function body and return type differ"; scopeUp(); {venv = venv, tenv = tenv, exp=TR.fundecIR(#exp(trbody), nm, nextLevel)}) end)
	      | trdec (venv, tenv, A.FunctionDec({name,params,body,pos,result}::fundeclist)) = (  let 
          val nextLevel = TR.newLevel({parent=level, name=name, formals=map(fn(field) => !(#escape field))(params)})
          val nm = funcDecl(name, params, result, nextLevel, pos);
	         val dec_ret = (trdec(venv, tenv, A.FunctionDec(fundeclist)));
          val rt = funcBody(name, params, result, pos, nextLevel);          
         val trbody = transExp(venv, tenv, body, break, nextLevel) in (if checkTypeEqual(rt, #ty(trbody)) then () else E.error pos "function body and return type differ"; scopeUp(); {venv = venv, tenv = tenv, exp=TR.combineFns(TR.fundecIR(#exp(trbody), nm, nextLevel), #exp dec_ret)}) end)				
	    in
    ...
    and transExp (venv, tenv, exp, break, level) = 
              ...
              | trexp (A.WhileExp({test, body, pos}), break) = (checkInt(trexp (test, break), pos); scopeDown(); loopdepth:= !loopdepth+1;
								...
								in
								    (if #ty(trbody)=T.UNIT then () else (E.error pos "while loop must be unit");
								     {exp = TR.whileIR(#exp (trexp (test, break)), #exp (trbody), newbreak), ty = T.UNIT})
								end)
		...
		in trexp (fwhile, break) 
              ...
              | trexp (A.SeqExp(exps), break) = let
                val exptys = map (fn(x) => (trexp((#1 x), break))) exps
                val exps = map (fn(x) => #exp(x)) exptys
                val tys = map (fn(x) => #ty(x)) exptys
              in
                 if List.length(tys) = 0 then {exp=TR.NIL, ty=T.UNIT} else {exp=TR.seqIR(exps), ty=List.last(tys)}
              end
              (*foldl (fn(x, y) => (trexp ((#1 x), break))) {exp=TR.Ex(Tree.CONST 0), ty=T.UNIT} exps*)
              | trexp (A.CallExp({func, args, pos}), break) = (case S.look(!venv, func) of SOME x =>
											 ...
											 | NONE => (E.error pos ("Function " ^ Symbol.name(func) ^ " undefined"); {exp = TR.nilIR(), ty = T.NIL}))
		...
		let val trtest = trexp(test, break)
		    val trthen = trexp(then', break)
		    val trelse = case else' of NONE => NONE
					     | SOME x => SOME(trexp(x, break))
		in
		    (checkInt(trtest, pos);
		     (case trelse of			  
              		NONE => if checkTypeEqual(#ty(trthen), T.UNIT) then () else E.error pos "If returns non-unit"
                      | SOME x => if (checkTypeEqual(#ty(trthen), #ty(x)) orelse checkTypeEqual(#ty(x), #ty(trthen))) then () else E.error pos "Then Else disagree");
		
                  {exp=TR.conditionalIR(#exp(trtest), #exp(trthen), 
                    case trelse of SOME(elseopt) => #exp(elseopt)
                      | NONE => TR.NIL
				       ), ty= #ty(trthen)})			
		end
    ...
    fun transProg(exp) = let
      val lbl = (TR.resetFragList();Temp.newlabel())
